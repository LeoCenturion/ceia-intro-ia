...
def dfs_tree_search(problem: hanoi_states.ProblemHanoi, display: bool = False):
    """
    ...
    explored = set()
    while frontier:
        node = frontier.pop()  # Extraemos el ultimo nodo de la pila
        if display: print(node)
        ...
        new_frontier_nodes = [child for child in expanded if child.state not in explored and child not in frontier]
        if display:
            for n in expanded:
                print("expanded: ", n)
            for n in explored:
                print("explored: ", n)
            for n in new_frontier_nodes:
                print("new frontier: ", n)
        frontier.extend(new_frontier_nodes)  # Agregamos a la cola todos los nodos sucesores del nodo actual
